...
/// <summary>
/// Ændring af login rate limit-policyen er en følsom "core policy"-handling (§3.2) og
/// kræver derfor step-up re-autentificering — håndhævet via StepUpGuard. Selve
/// forbruget (ReloadableLoginRateLimiter) reagerer allerede på ændringer via
/// IOptionsMonitor (se PR3) — denne editor giver blot admin-adgang til værdierne.
/// </summary>
public partial class LoginRateLimitEditor
{
    [Inject]
    private ILoginRateLimitPolicyAdminService AdminService { get; set; } = default!;
    ...
    private LoginRateLimitPolicyDto? settings;
    ...
    private async Task SaveCoreAsync()
    {
        ...
        try
        {
            ...
            await AdminService.UpdateAsync(new LoginRateLimitPolicyDto(permitEdit, windowEdit), userId);

            await LoadAsync();
            await OnStatusMessage.InvokeAsync("Login rate limit-policyen er opdateret og trådt i kraft.");
        }
